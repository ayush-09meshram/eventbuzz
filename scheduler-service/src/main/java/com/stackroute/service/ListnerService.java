...
public class ListnerService {
	...
	@KafkaListener(topics = "kafkashowjson", groupId = "showJson", containerFactory = "kafkaListenerContainerFactory")
	public void consumeKafka(Show show) throws ParseException, SchedulerException {
		...
		for (int i = 0; i < show.getMovies().size(); i++) {
			for (int j = 0; j < show.getMovies().get(i).getTheatres().size(); j++) {
				for (int k = 0; k < show.getMovies().get(i).getTheatres().get(j).getTimings().size(); k++) {
					...
					LocalDateTime ldt2 = ldt.minusMinutes(30);
		...
		for (int i = 0; i < schdStop.size(); i++) {
			...
			JobDetail job = JobBuilder.newJob(JobFunction.class).usingJobData("theatreId", schdStop.get(i).getTheatreId())
					.usingJobData("showTiming", schdStop.get(i).getShowTiming()).usingJobData("city", schdStop.get(i).getCity()).withIdentity(jobKey).build();
			...
			SimpleTrigger trigger = TriggerBuilder.newTrigger().forJob(job).withIdentity(triggerKey)
					.withDescription("Sample trigger").startAt(dateStart).withSchedule(SimpleScheduleBuilder.simpleSchedule())
					.build();
